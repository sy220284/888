...
`shellDialect` 选择 shell 栈（默认 `bash`，或 `pwsh`）：它决定默认的 `shellPath`／`shellArgs`（bash 为 `--noprofile --norc -i`；pwsh 经共享的 `dsh-pwsh-local` 解析器得到 `-NoLogo -NoProfile`）与启动契约。bash 方言通过环境安装提示符（`PS1` 加 OSC `133;D;` 终结的 `PROMPT_COMMAND`）。pwsh 无法从环境安装提示符，因此后端在启动 argv 后追加 `-NoExit -Command <bootstrap>`。该引导在交互输入循环开始前，把 `[Console]::OutputEncoding` 与 `$OutputEncoding` 钉为 UTF-8 并安装受控 `prompt` 函数，随后让同一进程继续运行；这样不会再通过 PSReadLine 抢跑写入启动输入，同时仍复用常规的 `LocalPtySession.initialize()` 就绪路径。pwsh 环境会去掉 bash 专属标记并加 `NO_COLOR`。两种方言发出相同的 BEL 终结 OSC 标记，因此就绪机制与消费方与方言无关。
