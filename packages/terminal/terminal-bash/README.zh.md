...
`shellDialect` 选择 shell 栈（默认 `bash`，或 `pwsh`）：它决定默认的 `shellPath`／`shellArgs`（bash 为 `--noprofile --norc -i`；pwsh 经共享的 `dsh-pwsh-local` 解析器得到 `-NoLogo -NoProfile`）与启动契约。bash 方言通过环境安装提示符（`PS1` 加 OSC `133;D;` 终结的 `PROMPT_COMMAND`）。pwsh 无法从环境安装提示符，因此后端先通过 `LocalPtySession.initialize()` 让未改动的交互式 host 到达原生提示符，再经已就绪的 PSReadLine 输入路径提交一次引导：把 `[Console]::OutputEncoding` 与 `$OutputEncoding` 钉为 UTF-8，安装受控 `prompt` 函数，并等待仅运行时可产生的 sentinel 加下一次输入就绪状态或受控提示符。这样既避免在初始提示符前抢跑写入，也避免启动时使用 `-Command` 改变 stdin 行为。pwsh 环境会去掉 bash 专属标记并加 `NO_COLOR`。两种方言发出相同的 BEL 终结 OSC 标记，因此就绪机制与消费方与方言无关。
