...
describe('hook-protocol invariants', () => {
  ...
  it('rejects unmatched and malformed results', async () => {
    ...
    session.append('hook/invoked', invoked({ handlerId: 'empty-message' }))
    expect(() => session.append('hook/result', result({ handlerId: 'empty-message', systemMessage: '' })))
      .toThrow(/systemMessage must be non-empty when present/)
