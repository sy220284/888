...
class FakeHandle implements NativeProcessHandle {
  ...
  alive = true
  async signalTree(signal: NativeExecutionSignal): Promise<void> { this.signals.push(signal); if (signal === 'SIGKILL') this.alive = false }
  async treeAlive(): Promise<boolean> { return this.alive }
}
...
class FakeNativeExecution extends NativeExecutionRuntime {
  ...
  async hello(): Promise<NativeExecutionHello> {
    return { protocol: 1, platform: 'test', capabilities: { processTree: true, terminal: false, filesystem: false, networkPolicy: false } }
  }
  async resolveExecutable(command: string): Promise<string> { return `/resolved/${command}` }
  spawn(spec: NativeProcessSpawnSpec): NativeProcessHandle { this.lastSpec = spec; return this.handle }
}
...
async function fixture(): Promise<{ ctx: Context; native: FakeNativeExecution; dispose(): Promise<void> }> {
  ...
  return {
    ...
    dispose: async () => { await subprocessFiber.dispose(); await nativeFiber.dispose() },
  }
}

describe('NativeSubprocessRuntime', () => {
  it.skipIf(process.platform === 'win32')('projects collected output with the same bounded-tail/spill semantics', async () => {
  ...
  it.skipIf(process.platform === 'win32')('scrubs ambient credentials while preserving deliberate explicit values and tombstones', async () => {
    process.env.NATIVE_AMBIENT_TOKEN = 'ambient-secret'
    process.env.NATIVE_VISIBLE = 'visible'
    const f = await fixture()
    try {
      f.ctx.subprocess.spawn(spec({ env: { NATIVE_AMBIENT_TOKEN: 'explicit', NATIVE_VISIBLE: undefined, DSH_EXPLICIT: 'yes' } }))
      ...
      f.native.handle.alive = false
      f.native.handle.doneState.resolve({ exitCode: 0, signal: null })
      f.native.handle.stdout.end(); f.native.handle.stderr.end()
      await f.dispose()
    }
  })

  it.skipIf(process.platform === 'win32')('escalates TERM to KILL against the whole native process tree', async () => {
    const f = await fixture()
    try {
      ...
      f.native.handle.doneState.resolve({ exitCode: null, signal: 'SIGKILL' })
      f.native.handle.stdout.end(); f.native.handle.stderr.end()
  ...
  it('does not claim PTY support before the native primitive exists', async () => {
    const f = await fixture()
    try {
      await expect(f.ctx.subprocess.spawnTerminal({
        argv: ['/bin/bash'], cwd: '/workspace', rows: 24, cols: 80, graceMs: 10,
      })).rejects.toThrow('terminal primitive is unavailable')
    } finally { await f.dispose() }
  })

  it.skipIf(process.platform === 'win32')('keeps the native handle managed after direct process exit until the tree is actually gone', async () => {
    const f = await fixture()
    try {
      const handle = f.ctx.subprocess.spawn(spec())
      f.native.handle.stdout.end(); f.native.handle.stderr.end()
      ...
      f.native.handle.alive = false
      await disposing
    } finally { f.native.handle.alive = false }
  })
})
