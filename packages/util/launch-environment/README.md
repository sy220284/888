...
- **No per-workspace layer** — the project layer is the _invoking_ directory, fixed at launch. A workspace selected later in the Web UI contributes nothing, deliberately: following it would let a model's own workspace change the harness environment mid-session.
