...
impl ManagedProcess {
    pub fn write_stdin(&self, bytes: &[u8]) -> Result<(), String> {
        let mut guard = self.stdin.lock().map_err(|_| "stdin lock poisoned".to_string())?;
        let stdin = guard.as_mut().ok_or_else(|| "stdin is not piped or is already closed".to_string())?;
        stdin.write_all(bytes).map_err(|e| format!("write stdin failed: {e}"))?;
...
pub fn resolve_executable(command: &str, env: &HashMap<String, String>) -> Result<PathBuf, String> {
    ...
    if command.contains('/') || command.contains('\\') {
        return Err(format!("relative executable paths are not allowed: {command:?}"));
    }
    let path_value = env.get("PATH").cloned().or_else(|| std::env::var("PATH").ok()).unwrap_or_default();
    for dir in std::env::split_paths(&path_value) {
        if dir.as_os_str().is_empty() { continue; }
        let candidate = dir.join(command);
        if verify_executable(&candidate).is_ok() {
            return candidate.canonicalize().map_err(|e| format!("canonicalize executable failed: {e}"));
        }
    }
...
fn verify_executable(path: &Path) -> Result<&Path, String> {
    let metadata = path.metadata().map_err(|e| format!("executable is not accessible: {e}"))?;
    if !metadata.is_file() {
...
pub fn spawn_process(
    ...
    let program = argv.first().filter(|v| !v.is_empty()).ok_or_else(|| "argv[0] must be non-empty".to_string())?;
    let mut command = Command::new(program);
    command.args(&argv[1..]).current_dir(&cwd).env_clear().envs(env);
    command.stdin(match stdin_mode { StdinMode::Ignore => Stdio::null(), StdinMode::Pipe | StdinMode::Data => Stdio::piped() });
    command.stdout(match stdout_mode { OutputMode::Pipe => Stdio::piped(), OutputMode::Ignore => Stdio::null() });
    command.stderr(match stderr_mode { OutputMode::Pipe => Stdio::piped(), OutputMode::Ignore => Stdio::null() });
    ...
    if matches!(stdin_mode, StdinMode::Data) {
        let write_result = (|| -> Result<(), String> {
            if let Some(data) = stdin_data {
                if let Some(stdin) = child_stdin.as_mut() {
                    stdin.write_all(&data).map_err(|e| format!("initial stdin write failed: {e}"))?;
                    stdin.flush().map_err(|e| format!("initial stdin flush failed: {e}"))?;
                }
            }
            Ok(())
        })();
        ...
        if let Err(error) = write_result {
            #[cfg(unix)] { let _ = signal_tree(pid, "SIGKILL"); }
    ...
    let managed = Arc::new(ManagedProcess { pid, stdin: Mutex::new(child_stdin) });
    match registry.lock() {
        Ok(mut guard) => { guard.insert(process_id.clone(), managed.clone()); }
        Err(_) => {
            #[cfg(unix)] { let _ = signal_tree(pid, "SIGKILL"); }
            #[cfg(not(unix))] { let _ = child.kill(); }
            let _ = child.wait();
    ...
    thread::spawn(move || {
        ...
        let (exit_code, signal) = match status {
            Ok(status) => {
                #[cfg(unix)]
                {
                    ...
                    (status.code(), status.signal().and_then(signal_name).map(str::to_string))
                }
                #[cfg(not(unix))]
                {
                    (status.code(), None)
        ...
        if let Some(process) = registry.lock().ok().and_then(|guard| guard.get(&process_id).cloned()) {
            let _ = process.close_stdin();
        }
        let event = Event::Exit { process_id: process_id.clone(), exit_code, signal };
        ...
        if let Ok(mut guard) = registry.lock() { guard.remove(&process_id); }
    });

    Ok(managed)
}

fn stream_output<R: Read + Send + 'static>(process_id: String, stream_name: &'static str, mut stream: R, writer: Writer) {
    thread::spawn(move || {
        ...
        loop {
            ...
            let event = if stream_name == "stdout" {
                Event::Stdout { process_id: process_id.clone(), data_b64 }
            } else {
                Event::Stderr { process_id: process_id.clone(), data_b64 }
            };
            if write_json(&writer, &event).is_err() {
                break;
            }
        }
        let _ = write_json(&writer, &Event::StreamClosed { process_id, stream: stream_name });
    });
}
...
pub fn write_json<T: serde::Serialize>(writer: &Writer, value: &T) -> Result<(), String> {
    let mut guard = writer.lock().map_err(|_| "protocol writer lock poisoned".to_string())?;
    serde_json::to_writer(&mut *guard, value).map_err(|e| format!("serialize response failed: {e}"))?;
    guard.write_all(b"\n").map_err(|e| format!("write response failed: {e}"))?;
    guard.flush().map_err(|e| format!("flush response failed: {e}"))
}
...
#[cfg(not(unix))]
pub fn tree_alive(_pid: u32) -> bool { false }
