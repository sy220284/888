...
need() { command -v "$1" >/dev/null 2>&1 || {
	echo "missing bootstrap dependency: $1" >&2
	exit 1
}; }
need curl
need tar
...
Darwin)
	...
	;;
*)
	echo "unsupported bootstrap OS: $os" >&2
	exit 1
	;;
esac
case "$arch" in
...
aarch64 | arm64)
	...
	;;
*)
	echo "unsupported bootstrap architecture: $arch" >&2
	exit 1
	;;
esac
...
download_node() {
	...
	[ -n "$expected" ] || {
		echo "Node checksum entry not found: $archive" >&2
		exit 1
	}
	actual=$(sha256_file "$archive_path")
	[ "$actual" = "$expected" ] || {
		echo "Node SHA-256 mismatch" >&2
		exit 1
	}
	printf '%s\n' "$archive_path"
}
...
install_uv() {
	...
	command -v "$py" >/dev/null 2>&1 || {
		echo "Python >=3.10 is required for the python/full profile" >&2
		exit 1
	}
