...
The install resolves project dependencies without changing Git configuration or installing repository hooks.

Run typecheck manually when a checkout needs validation:

```sh
pnpm run typecheck
```

The command is optional and does not run during install, commit, or push.
...
`pnpm run build` embeds the caller's exact `DSH_CLIENT_*` environment and uses no public client values when none are set. `pnpm run build:official` creates the official release artifact profile. Each successful complete build writes a gitignored record that binds those values to the Vite output and dynamic client bundles; release packing and built Web tests reject a missing record or artifacts changed by a later partial build.

Static analysis and tests resolve workspace imports through the base `paths` map to `src`; commands that consume built `lib/` output declare that dependency explicitly. Generated Host-for-Client Remote declarations are the deliberate exception: the public `typecheck`, `lint`, and `doc-typecheck` commands generate them first, while internal `*:contracts-ready` scripts assume that the caller already ran the Typert declaration-generation pass or a complete build. See the [solution-root note](../.agents/notes/implemented/process/2026-07-22-tsconfig-solution-root-two-aggregates.md) for the two-aggregate setup, the [ts-build-config note](../.agents/notes/implemented/process/2026-06-17-ts-build-config.md) for tsc-first emit ownership, and the [Typert Remote note](../.agents/notes/implemented/architecture/2026-08-02-typert-remote-method-calls.md) for declaration preparation.
...
The repository defines no Git hooks, merge drivers, issue-policy automation, or GitHub Actions workflows. Installation, commit, merge, and push operations do not invoke project checks.

Validation commands remain available for explicit local use. `pnpm run check:all` runs the broad manual check set, while `test`, `typecheck`, `lint`, `doc-sync`, and `hygiene` run narrower checks.

### Daily commands

The root [contributor instructions](../AGENTS.md#commands) summarize common commands, while [`package.json`](../package.json) and [scripts/run-checks.ts](../scripts/run-checks.ts) own the manual check inventory. Select the smallest useful check for the changed code. Built-artifact checks require `pnpm run build` first.
